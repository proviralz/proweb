...
const Register = () => {
    ...
    const registerUser = async ()=> {
        try {
            setIsClicked(true)
            const res = await publicRequest.post('auth/admin/register', {
                fullName: name,
                email: email,
                password: password
            })
    ...
    <div className=' relative md:flex'>
        ...
        <div className=' p-10 absolute top-0 left-0'>
            <div>
                ...
                <p className=' font-bold text-fuchsia-950 text-2xl'>
                    Admin
                </p>
            </div>
        </div>
                ...
                <div className=' mt-10 md:mt-0 bg-white h-full p-5 md:p-10  md:shadow-2xl'>
                    ...
                    <div className=' mt-10 flex justify-center'>
                        <p className=' text-neutral-500 font-light'>
                            Already Registered? <Link href={'/admin/login'} className=' text-[#31013f] '>Login</Link>
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div className=' hidden w-1/2 bg-[#e1dddd] h-screen md:flex items-center justify-center'>
            <Image src={'/assets/admin/bro.svg'} priority={true} alt='login' width={100} height={100} className=' w-full'  />
       </div>
    </div>
  )
}
