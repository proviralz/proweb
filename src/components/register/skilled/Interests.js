...
const Interests = () => {
    ...
    useEffect(() => {
        ...
        setFilteredInterests(skilledInterest);
      }, []);

    useEffect(() => {
        const results = skilledInterest.filter(interest =>
                    ...
                    <div className=' mt-5 w-full h-96 overflow-y-scroll'>
                        <div className=' flex flex-wrap gap-5 justify-between'>
                            {filteredInterests.map((u, i)=> (
                                <div 
                                    key={i}  
                                    className={`w-36 h-52 border p-3 rounded-lg ${selectedInterests.includes(u.title) ? 'bg-gray-300' : ''} `} 
                                    onClick={() => toggleInterest(u.title)}>
